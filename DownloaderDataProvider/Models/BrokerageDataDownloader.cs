    ...
    public class BrokerageDataDownloader : IDataDownloader
    {
        ...
        public BrokerageDataDownloader()
        {
            ...
            try
            {
                ...
                var brokerageFactory = Composer.Instance.Single<IBrokerageFactory>(factory =>
                    factory.BrokerageType.MatchesTypeName(liveNodeConfiguration.Brokerage));
                liveNodeConfiguration.BrokerageData = brokerageFactory.BrokerageData;
            }
            catch (InvalidOperationException error)
            {
                throw new InvalidOperationException(
                    $"{nameof(BrokerageDataDownloader)}.An error occurred while resolving brokerage data for a live job. Brokerage: {liveNodeConfiguration.Brokerage}.",
                    error);
            }
        ...
        public IEnumerable<BaseData>? Get(DataDownloaderGetParameters dataDownloaderGetParameters)
        {
            ...
            Console.WriteLine($"BrokerageDataDownloader.get {symbol.Value} {symbol.IsCanonical()}");
            if (symbol.IsCanonical())
            {
                symbols = GetChainSymbols(symbol, true).ToList();
                Console.WriteLine($"GetChainSymbols is {symbols.Select(s => s.Value)}");
            }
            
            return symbols
                .Select(symbol =>
                {
                    var request = new Data.HistoryRequest(startUtc, endUtc, dataType, symbol, resolution,
                        exchangeHours: exchangeHours, dataTimeZone: dataTimeZone, resolution,
                        includeExtendedMarketHours: true, false, DataNormalizationMode.Raw, tickType);
                    IEnumerable<BaseData> history = _brokerage.GetHistory(request);
        ...
        {
            Console.WriteLine($"GetChainSymbols {this._brokerage.GetType().Name}");
